...
4.  The [`IConstraint`](xref:SudokuSpice.Constraints.IConstraint)
  ...
  [`SquareLink`s](xref:SudokuSpice.Data.SquareLink), which represent the 1s in the matrix. Each
  [`SquareLink`] is connected up and down to the other '1s' that satisfy that constraint header,
  and connected left and right to the other '1s' that are present for that possible square value.
